...
pub struct OutputUnit {
    ...
    pub object: Option<Utf8UnixPathBuf>,
    pub name: String,
...
fn split_write_obj(
    ...
    for (unit, split_obj) in module.obj.link_order.iter().zip(&split_objs) {
        // Skip externally compiled units — their .o comes from the build system, not from us.
        if !unit.link_only_sections.is_empty() {
        ...
        out_config.units.push(OutputUnit {
            object: Some(out_path.with_unix_encoding()),
            name: unit.name.clone(),
            autogenerated: unit.autogenerated,
    ...
    if config.write_asm {
        ...
        for (unit, split_obj) in module.obj.link_order.iter().zip(&split_objs) {
            // No asm to write for externally compiled units
            if !unit.link_only_sections.is_empty() {
                continue;
            }
