...
fn resolve_link_order(obj: &ObjInfo) -> Result<Vec<ObjUnit>> {
    ...
    match toposort(&graph) {
        Ok(vec) => {
            let mut result: Vec<ObjUnit> = vec
                .iter()
                .map(|&idx| {
            ...
            // Append any units from link_order that had no splits (externally compiled).
            // These have no binary range, so they never entered unit_to_index_map
            // and won't appear in the toposort result. They must come last.
            for unit in &obj.link_order {
                if !result.iter().any(|u| u.name == unit.name) {
