...
where W: Write + ?Sized {
    ...
    for unit in obj.link_order.iter().filter(|unit| all || !unit.autogenerated) {
        ...
        for section_name in &unit.link_only_sections {
            writeln!(w, "\t{}", section_name)?;
        }
    }
    Ok(())
}
...
fn parse_section_line(captures: Captures, state: &SplitState) -> Result<SplitLine> {
    ...
    for attr in captures["attrs"].split(' ').filter(|&s| !s.is_empty()) {
        if let Some((attr, value)) = attr.split_once(':') {
            match attr {
                "start" => section.start = Some(parse_u32(value)?),
                "end" => section.end = Some(parse_u32(value)?),
    ...
    match (start, end) {
        ...
        _ => Err(anyhow!(
            "Section '{}' must have both start and end, or neither",
            section.name
        )),
    }
}
        ...
        match (&mut state, split_line) {
            ...
            (
                ...
                if start.is_none() && end.is_none() {
                    // Rangeless: unit is already in link_order, just record the section name.
                    if let Some(u) = obj.link_order.iter_mut().find(|u| u.name == *unit) {
                        u.link_only_sections.push(name.clone());
                    }
                } else {
